...
class dpsAnalysis:
    ...
    def init_files_P(self):
        ...
        file_base += "pointProyect/dpsAnalysis/voxel/data/P_"
        for dsp_type in self.parSer.dsp_types:
            file_tmp = file_base + dsp_type + ".txt"
            with open(file_tmp, 'w') as f:
                f.write("voxel P12 P13 P32\n")
    ...
    def save_data_P_dps_type(self, dsp_type, voxel, P12, P13, P32):
        file = ""
        file += self.parSer.prefix
        file += "pointProyect/dpsAnalysis/voxel/data/P_"
        file += dsp_type + ".txt"
        with open(file, 'a') as f:
            f.write(
                str(voxel)+" " + str(P12)+" " + str(P13)+" "+str(P32)+"\n")

    def save_data_P_time(self, voxel, time):
        file = ""
        file += self.parSer.prefix
        file += "pointProyect/dpsAnalysis/voxel/data/P_time.txt"
        with open(file, 'a') as f:
            f.write(str(voxel)+" " + str(time)+"\n")

    def voxels(self, voxelSize_init, voxelSize_finish, voxelSize_steps):
        print("voxels")
        for voxelSize in np.arange(voxelSize_init, voxelSize_finish, voxelSize_steps):
            time_inicio = tm.time()
            print(">"*10)
            print("-> voxelSize: ", voxelSize)
            print("-> calculo de matrices de covarianza")
            self.pcd_xyz.estimate_covariances(
                search_param=o3d.geometry.KDTreeSearchParamRadius(radius=voxelSize))
            ...
            for dsp_type in self.parSer.dsp_types:
                ...
                self.save_data_P_dps_type(dsp_type, voxelSize, P12, P13, P32)
            e = None
            dsp_value_tmp = None
            time = tm.time() - time_inicio
            self.save_data_P_time(voxelSize, time)
    ...
    def view_grafico_gauss(self):
        ...
        pwd_imagen += "pointProyect/dpsAnalysis/voxel/images/"

        pwd_files = ""
        pwd_files += self.parSer.prefix
        pwd_files += "pointProyect/dpsAnalysis/voxel/data/"
        ...
        for name_file in name_files:
            ...
            plt.figure()
            plt.plot(voxel, P12, label="Arbol_suelo")
            plt.plot(voxel, P13, label="Arbol_Marcador")
            plt.plot(voxel, P32, label="Marcador_Suelo")
            plt.title(name_x + "_vs_voxel")
            plt.xlabel('voxel')
            plt.ylabel('P')
            plt.grid(True)
            plt.legend()
            plt.savefig(pwd_imagen + name_x + "_vs_voxel.png")
            plt.clf()
        ...
        voxel = np.array(times.voxel)
        time = np.array(times.time)

        plt.figure()
        plt.plot(voxel, time, label="time")
        plt.title("time_vs_voxel")
        plt.xlabel('voxel')
        plt.ylabel('time')
        plt.grid(True)
        plt.legend()
        plt.savefig(pwd_imagen + "time_vs_voxel.png")
        plt.clf()
...
if __name__ == '__main__':
    ...
    if opcion == "1":
        ...
        print("-"*10)
        voxel_init = float(input("voxel_init: "))
        voxel_finish = float(input("voxel_finish: "))
        voxel_steps = float(input("voxel_steps: "))
        print("-"*10)
        dps_analysis.voxels(voxel_init, voxel_finish, voxel_steps)
    elif opcion == "2":
        print("="*10)
        print(" graficas P")
